#!/bin/bash
# DEPRECATED: Legacy external loop for Oracle research.
#
# The preferred method is now the in-session Stop hook loop, which provides
# full conversation context between iterations for dramatically better research.
# See: .aether/utils/oracle/oracle-stop-hook.sh
#
# This script remains functional for:
# - Environments where Stop hooks are not available
# - Explicit tmux-based background research (--legacy flag)
# - Debugging the loop mechanism
#
# To use legacy mode: bash .aether/utils/oracle/oracle.sh
# To use in-session mode: /ant:oracle (automatic)
#
# ---
