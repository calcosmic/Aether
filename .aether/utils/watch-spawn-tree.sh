...
EOF
  ...
  # Show Prime Worker if any workers exist
  if [[ ${#workers[@]} -gt 0 ]]; then
    echo -e "  ${DIM}└──${RESET} ${BOLD}🐜 Prime Worker${RESET}"
  fi
  ...
  for name in "${roots[@]}"; do
    ...
    echo -e "      ${DIM}├──${RESET} ${emoji} ${color}${name}${RESET}: ${task} ${DIM}[${status}]${RESET}"

    # Find children of this worker
    for child in "${!workers[@]}"; do
      if [[ "${workers[$child]}" == "$name" ]]; then
        [[ " ${printed[*]} " =~ " $child " ]] && continue
        printed+=("$child")

        c_emoji=$(get_emoji "${worker_caste[$child]}")
        c_status="${worker_status[$child]}"
        c_color=$(get_status_color "$c_status")
        c_task="${worker_task[$child]}"
        [[ ${#c_task} -gt 30 ]] && c_task="${c_task:0:27}..."

        echo -e "      ${DIM}│   └──${RESET} ${c_emoji} ${c_color}${child}${RESET}: ${c_task} ${DIM}[${c_status}]${RESET}"
      fi
    done
  done
