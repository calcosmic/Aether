...
Log activity: aether activity-log --command "sage" --result "analyzing" --details "{sage_name} analytics review"
...
# Auto-promote all eligible observations to instincts in one call.

auto_result=$(aether learning-promote-auto 2>/dev/null || echo '{}')
auto_promoted_count=$(echo "$auto_result" | jq -r '.result.promoted // 0' 2>/dev/null || echo "0")

if [[ "$auto_promoted_count" -gt 0 ]]; then
  echo "Auto-promoted $auto_promoted_count observation(s) to instincts (met recurrence thresholds)"
  echo ""
fi
# --- END Batch auto-promotion ---

# Check for pending proposals
proposals=$(aether learning-check-promotion --all 2>/dev/null || echo '{"proposals":[]}')
...
```bash
aether activity-log --command "seal" --phase "$current_phase" --result "completed" --details "Colony sealed - wisdom review completed"
```
...
aether changelog-append \
  --date "$(date +%Y-%m-%d)" \
  --plan "seal-crowned-anthill" \
  --phase "00" \
  --entry "Colony sealed at Crowned Anthill;{goal};{phases_completed} phases completed;Colony wisdom promoted to QUEEN.md"
```
...
Log activity: aether activity-log --command "chronicler" --result "surveying" --details "{chronicler_name} documentation audit"
...
```bash
seal_commit=$(aether generate-commit-message --type seal --subject "$phases_completed phases completed: $goal" --scope "colony" --body "Colony age: ${colony_age_days} days" 2>/dev/null)
