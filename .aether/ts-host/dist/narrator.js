 ...
 */
import { loadCeremonyConfig, } from "./caste-config.js";
...
export function createNarrator(opts) {
    ...
    const handlers = {
        ...
        "ceremony.oracle.phase_transition": (payload) => renderer.renderStageSeparator(`Oracle: ${payload.status ?? "unknown"}`, config),
        "ceremony.oracle.iteration": (payload) => renderer.renderSpawnFrame({
            caste: "oracle",
...
// Content builders
// ---------------------------------------------------------------------------
function buildSummaryContent(payload) {
    const parts = [];
...
function buildCloseoutContent(payload) {
    const parts = [];
