...
const warnedTimeoutValues = new Set();
/** Test-only: reset the once-per-value warning dedupe. */
export function __resetPreflightTimeoutWarnings() {
...
export function resolvePreflightTimeoutMs() {
    ...
    if (!warnedTimeoutValues.has(raw)) {
        ...
        process.stderr.write(`Warning: AETHER_PREFLIGHT_TIMEOUT="${raw}" is not a positive duration (expected e.g. "90s", "1500ms", "1m30s", "1.5h") — falling back to ${PREFLIGHT_DEFAULT_TIMEOUT_MS}ms\n`);
    }
    return PREFLIGHT_DEFAULT_TIMEOUT_MS;
}
