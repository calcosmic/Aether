...
import { toWorkerResults } from "../worker-dispatch.js";
import { deriveWorkflowPattern, formatQueenRecommendation, } from "./workflow-patterns.js";
import { applyBuilderProbeLock, isBuilderProbeLockSatisfied, } from "./builder-probe-lock.js";
 ...
 */
export function createQueenOrchestrator(opts) {
    return {
 ...
 */
export async function runBuild(opts, manifest) {
    ...
    process.stderr.write(`Queen: recommendation = ${formatQueenRecommendation(recommendation)}\n`);
    ...
    // ── Step 6: Build result ─────────────────────────────────────────────────
    const success = failures.length === 0 && isBuilderProbeLockSatisfied(workerResults, dispatches);
    return {
        success,
        workerResults,
        pattern,
 ...
 */
function buildRecommendation(pattern, dispatches) {
