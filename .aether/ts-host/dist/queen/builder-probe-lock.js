 ...
 */
export function applyBuilderProbeLock(results, dispatches) {
 ...
 */
export function hasProbeVerification(results, dispatches) {
    const dispatchByName = new Map();
    for (const d of dispatches) {
        dispatchByName.set(d.name, d);
    }
 ...
 */
export function isBuilderProbeLockSatisfied(results, dispatches) {
    const hasBuilder = dispatches.some((d) => d.caste === "builder");
    const hasProbe = dispatches.some((d) => d.caste === "probe" || d.caste === "watcher");
