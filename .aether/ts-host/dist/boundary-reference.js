 ...
 */
export const RUNTIME_BOUNDARY_CONTRACT_PATH = ".aether/references/contracts/runtime-boundary-contract.md";
/**
 ...
 */
export function isReadOnlyAllowed(path) {
 ...
 */
export function assertNoWriteToData(path, mode) {
    const normalized = path.replace(/\\/g, "/");
    // Not under .aether/data/ — no concern
    if (!normalized.startsWith(".aether/data/"))
        return;
    // Read mode on allowlisted path — permitted
    if (mode === "read" && isReadOnlyAllowed(normalized))
        return;
    // Everything else under .aether/data/ is rejected
    throw new BoundaryViolationError(`TS host attempted to write to Go-owned path: ${path}`);
}
