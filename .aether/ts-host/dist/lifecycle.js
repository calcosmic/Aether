...
export function __setCreateQueenOrchestrator(fn) {
    _createQueenOrchestratorRef = fn;
}
/** Test-only: restore the real createQueenOrchestrator. */
export function __restoreCreateQueenOrchestrator() {
    _createQueenOrchestratorRef = _createQueenOrchestrator;
}
function createQueenOrchestrator(...args) {
...
export async function runLifecycle(opts) {
    if (opts.simulateWorkers !== true) {
        return {
            success: false,
            steps_completed: [],
            error: "Lifecycle host is experimental and simulate-only; pass simulateWorkers=true for the smoke harness or run aether plan/build/continue for real orchestration.",
        };
    }
    ...
    try {
        ...
        if (opts.runOracle) {
            ...
            process.stderr.write(`Oracle completed: ${oracleRes.iterations_completed} iterations, confidence ${oracleRes.final_confidence}%\n`);
        }
        // ── Step 1: Plan ─────────────────────────────────────────────────────
        const planResult = callGoJSON(opts, [
            "plan",
            "--plan-only",
            "--depth",
            "fast",
        ...
        // ── Step 2: Build ────────────────────────────────────────────────────
        const buildResult = callGoJSON(opts, [
            "build",
            String(targetPhase),
            "--plan-only",
        ]);
        ...
        if (!buildManifest) {
            throw new Error("Build --plan-only returned no dispatch_manifest");
        }
        ...
        if (simulateWorkers) {
            ...
            try {
                ...
                writeFileSync(join(opts.cwd, placeholderRel), "Simulated build output for TS host prototype lifecycle test.\n", "utf-8");
            }
            catch {
        ...
        const queenOpts = {
            ...
            ...(simulateWorkers ? { simulatedFileClaims: [".aether/ts-host/SIMULATED_BUILD_OUTPUT.txt"] } : {}),
        };
        ...
        if (queenResult.middenResult?.exceeded) {
            process.stderr.write(`Warning: midden threshold exceeded (${queenResult.middenResult.total} entries)\n`);
        }
        if (queenResult.recoveryActions && queenResult.recoveryActions.length > 0) {
            process.stderr.write(`Recovery actions: ${queenResult.recoveryActions.length} action(s) generated\n`);
        }
        ...
        const buildCompletionPath = writeCompletionFile(approvedCompletionDirPrefix("build"), "build-completion.json", { result: buildCompletion });
        ...
        // ── Step 3: Continue ─────────────────────────────────────────────────
        const continueResult = callGoJSON(opts, [
            "continue",
            "--plan-only",
        ]);
        ...
        if (!continueManifest) {
            throw new Error("Continue --plan-only returned no continue_manifest");
        }
        ...
        renderManifestCeremony(ceremony, "continue", continueCeremonyEnvelope, continueDispatches);
        ...
        const continueResults = continueDispatches.map((d) => ({
            ...d,
            status: "completed",
        ...
        const continueCompletionPath = writeCompletionFile(approvedCompletionDirPrefix("continue"), "continue-completion.json", { result: continueCompletion });
        // Call continue-finalizer to run verification, gates, and advance state
        const continueFinalizeResult = callGoJSON(opts, [
            "continue-finalize",
            "--completion-file",
            continueCompletionPath,
        ]);
        emitCeremonyOutput(ceremony.renderCloseout("continue", continueCompletionPath));
        // Check if continue was blocked by gates (informational, not a failure)
        const blocked = continueFinalizeResult["blocked"] === true;
        if (blocked) {
            process.stderr.write("Continue blocked by verification gates (informational)\n");
        }
