...
import { assertNoWriteToData, BoundaryViolationError, } from "./boundary-reference.js";
// ---------------------------------------------------------------------------
// State guards
// ---------------------------------------------------------------------------
 ...
 */
function guardWriteMode(path, mode) {
 ...
 */
export async function startEventBridge(opts) {
    ...
    let replayEvents = [];
    try {
        const replayResult = callGoJSON(opts, replayArgs);
    ...
    rl.on("line", (line) => {
        ...
        if (!isValidCeremonyEvent(parsed))
            return;
        const evt = parsed;
        if (evt.id && seen.has(evt.id))
            return;
 ...
 */
export async function stopEventBridge(controller) {
    await controller.stop();
}
// ---------------------------------------------------------------------------
// Validation
// ---------------------------------------------------------------------------
function isValidCeremonyEvent(obj) {
    if (typeof obj !== "object" || obj === null)
        return false;
    const e = obj;
    if (typeof e.id !== "string")
        return false;
    if (typeof e.topic !== "string")
        return false;
