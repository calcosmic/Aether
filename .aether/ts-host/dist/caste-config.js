...
// ---------------------------------------------------------------------------
export const DEFAULT_CEREMONY_CONFIG = {
    castes: {
...
// Validation helpers
// ---------------------------------------------------------------------------
function isValidCasteConfig(obj) {
 ...
 */
export function loadCeremonyConfig(cwd) {
    ...
    for (const [name, cfg] of Object.entries(castes)) {
        if (!isValidCasteConfig(cfg)) {
            throw new Error(`Invalid ceremony.yaml: caste "${name}" missing emoji, color, or label`);
        }
    }
    return parsed;
}
 ...
 */
export function getCasteConfig(config, casteName) {
    return config.castes[casteName];
}
/**
 * Get a caste's emoji.
 */
export function getCasteEmoji(config, casteName) {
    return config.castes[casteName]?.emoji ?? "❓";
}
/**
 * Get a caste's color (hex).
 */
export function getCasteColor(config, casteName) {
    return config.castes[casteName]?.color ?? "#FFFFFF";
}
/**
 * Get a caste's label.
 */
export function getCasteLabel(config, casteName) {
