...
async function _realDetectAvailablePlatforms() {
    const available = [];
 ...
 */
export function formatPlatformUnavailableMessage(context = "worker dispatch", providerDiagnostics) {
    ...
    return (`Worker dispatch cannot start for ${context}; the TypeScript host did not receive ` +
 ...
 */
export function formatPlatformDiagnosticMessage(platform) {
    switch (platform) {
        case "claude":
 ...
 */
export function classifyPlatformError(_platform, error) {
 ...
 */
export async function isPlatformAvailable(platform) {
    const binary = resolveBinaryName(platform);
 ...
 */
export function createPlatformDispatcher(platform) {
    return {
        platform,
        spawnWorker: (config) => spawnWorker({ ...config, platform }),
    };
}
// ---------------------------------------------------------------------------
// Internal helpers
...
async function runProbe(binary, args) {
    let lastError;
 ...
 */
function runProbeOnce(binary, args) {
...
function codexLoginStatusIsActive(output) {
