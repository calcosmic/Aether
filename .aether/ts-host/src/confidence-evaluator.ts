...
export class ConfidenceEvaluator {
  ...
  evaluate(input: ConfidenceInput): EvaluatedConfidence {
    ...
    for (const claim of claims) {
      ...
      const raw = claim as unknown as Record<string, unknown>;
      if (
        typeof raw.test_results === "object" &&
        raw.test_results !== null
      ) {
        const tr = raw.test_results as Record<string, unknown>;
        if (typeof tr.passed === "number") totalPassed += tr.passed;
        if (typeof tr.total === "number") totalTests += tr.total;
      }
    }
    ...
    for (const claim of claims) {
      if (claim.files_created) {
        for (const f of claim.files_created) fileSet.add(f);
      }
      if (claim.files_modified) {
        for (const f of claim.files_modified) fileSet.add(f);
      }
    }
    ...
    for (const claim of claims) {
      if (claim.blockers) {
        for (const b of claim.blockers) blockers.push(b);
      }
    }
    ...
    // Source detection
    const source: "worker-claims" | "finalizer-gate" =
      input.finalizerResult !== undefined &&
