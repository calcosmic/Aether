...
// ---------------------------------------------------------------------------
// Evidence-based research scorer (D-11)
// ---------------------------------------------------------------------------
...
// against.
const REQUIRED_SECTIONS: readonly string[] = Object.freeze([
...
const CITED_SECTIONS: readonly string[] = Object.freeze([
  "## Key Patterns",
  "## Gotchas",
]);
...
function isCited(bullet: string): boolean {
  ...
  if (/https?:\/\//.test(sourceSpan)) return true;
...
function extractPath(bullet: string): string {
  ...
  evaluate(input: ResearchEvidenceInput): EvaluatedResearchConfidence {
    ...
    for (const heading of CITED_SECTIONS) {
      ...
      for (const bullet of bullets) {
        citationsTotal++;
        if (isCited(bullet)) citationsVerified++;
      }
    }
    const citationScore =
      citationsTotal > 0
    ...
    const filesScore =
      filesTotal > 0 ? FILES_BONUS_MAX * (filesVerified / filesTotal) : 0;
