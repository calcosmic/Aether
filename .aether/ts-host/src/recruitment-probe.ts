...
const warnedRecruitmentProbeTimeoutValues = new Set<string>();

/** Test-only: reset the once-per-value warning dedupe. */
export function __resetRecruitmentProbeTimeoutWarnings(): void {
 ...
 */
export function resolveRecruitmentProbeTimeoutMs(): number {
  const raw = process.env["AETHER_RECRUIT_PROBE_TIMEOUT"]?.trim();
  if (!raw) return RECRUITMENT_PROBE_DEFAULT_TIMEOUT_MS;
  ...
  if (!warnedRecruitmentProbeTimeoutValues.has(raw)) {
    ...
    process.stderr.write(
      `Warning: AETHER_RECRUIT_PROBE_TIMEOUT="${raw}" is not a positive duration (expected e.g. "10s", "1500ms", "1m30s", "1.5h") — falling back to ${RECRUITMENT_PROBE_DEFAULT_TIMEOUT_MS}ms\n`
    );
  }
...
const PROBE_PROMPT =
  "List your available tools, then reply with exactly one line and nothing else: NATIVE_NESTING_AVAILABLE=true if an agent-spawning tool (for example, a Task tool) appears in your own available tool list, otherwise NATIVE_NESTING_AVAILABLE=false.\n";

function nativeNestingReportedAvailable(output: string): boolean {
...
export type RecruitmentProbeLauncher = (
  binary: string,
  args: string[],
  cwd: string,
  timeoutMs: number
) => Promise<RecruitmentProbeLaunchResult>;

async function defaultRecruitmentProbeLauncher(
  binary: string,
  args: string[],
  cwd: string,
  timeoutMs: number
...
let _launcherRef: RecruitmentProbeLauncher = defaultRecruitmentProbeLauncher;

/** Test-only: substitute a deterministic launcher. */
export function __setRecruitmentProbeLauncher(fn: RecruitmentProbeLauncher): void {
  _launcherRef = fn;
}

/** Test-only: restore the real launcher. */
export function __restoreRecruitmentProbeLauncher(): void {
  _launcherRef = defaultRecruitmentProbeLauncher;
}

function resolveRecruitmentProbeBinary(): string {
...
function recruitmentProbeArgs(): string[] {
...
let cachedProbe: RecruitmentProbeResult | undefined;
let cachedProbePromise: Promise<RecruitmentProbeResult> | undefined;

/** Test-only: clear the session cache so a test can exercise both the first
 * (real launch) and the cached (no launch) call. */
export function __resetRecruitmentProbeCache(): void {
 ...
 */
export async function probeNativeNesting(): Promise<RecruitmentProbeResult> {
...
async function runRecruitmentProbe(): Promise<RecruitmentProbeResult> {
  ...
  let attemptResult: RecruitmentProbeLaunchResult | undefined;
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      attemptResult = await _launcherRef(binary, recruitmentProbeArgs(), dir, budgetMs);
      if (!attemptResult.timedOut) break;
