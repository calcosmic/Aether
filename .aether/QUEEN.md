...
- Communication style: Plain English first
- Plain-English explanations: include a short for-dummies explanation alongside technical detail.
- Explain what changed, why it matters, and what it means for the user before deeper implementation detail.
- Prioritize: Correct host dispatch
- Avoid: Do not default unknown hosts to Codex
...
- Tech stack and tools: Go CLI
- Canonical commands: go test ./cmd ./pkg/codex
- Verification baseline: Build plus targeted and package tests
- Store durable repo facts here. Do not use QUEEN.md for temporary instructions, secrets, or one-off task notes.
...
- **Project:** Cross-platform Queen brain
- **Current Development:** Claude OpenCode Codex parity
- **Audience:** Aether users
- **Source:** Generated by `aether queen-compose` on 2026-05-03.
- **Authority:** Treat this as durable repo context. Current user requests, active REDIRECT signals, blockers, and runtime state override this file.

## Project Brief

- Purpose: Cross-platform Queen brain
- Current work: Claude OpenCode Codex parity
- Primary users: Aether users
- Keep this section concise and factual; move long procedures into command playbooks, docs, or skills.

## Philosophies

- Correct host dispatch
...
- Provider auth hardening must cover both preflight availability checks and post-launch worker stdout/stderr publication paths. (seal review phase 5, 2026-05-16)
- Provider/auth redaction belongs at publication boundaries after parsing, covering errors, RawOutput, debug artifacts, and TS summaries. (seal review phase 5, 2026-05-16)
- Provider/auth redaction tests should include both preflight probe failures and post-launch worker stdout, stderr, parse-error, RawOutput, and debug-artifact paths. (seal review phase 5, 2026-05-16)
- Release handoffs should distinguish verified scanner output from residual risk; do not claim govulncheck passed when the tool was unavailable. (seal review phase 5, 2026-05-16)
- Seal evidence should capture exact command rows for every required release smoke, not just worker and event summaries. (seal review phase 5, 2026-05-16)
- Security ledger entries need an explicit resolution path after builders fix findings; stale open HIGH entries make seal review ambiguous. (seal review phase 5, 2026-05-16)
- A real smoke can be acceptable when external auth blocks it, but only if dispatch was real, continue blocked, and the exact failure is recorded. (seal review phase 4, 2026-05-15)
- A smoke failure can be seal-safe when the runtime fails closed and records the exact limitation. (seal review phase 4, 2026-05-15)
- Provider auth output should be treated as environment/setup failure, not worker-result parse failure. (seal review phase 4, 2026-05-15)
- Real colony smoke should preserve environment failures as explicit known issues when the runtime blocks advancement correctly. (seal review phase 4, 2026-05-15)
- Use isolated HOME, hub, binary, and temp repo for lifecycle smoke to avoid contaminating the source checkout. (seal review phase 4, 2026-05-15)
- Verification-only build phases need provenance rules that accept implementation-worker output evidence without weakening mutation-phase file-change requirements. (seal review phase 4, 2026-05-15)
- Verification-only provenance should stay explicit; avoid relying only on task wording over time. (seal review phase 4, 2026-05-15)
- Wrapper contract reviews should pair Go finalizer/provenance tests with TS host flag forwarding tests because either side can silently break lifecycle delegation. (seal review phase 4, 2026-05-15)
- Wrapper contract tests should assert TS-host delegation plus visual closeout, not stale direct plan-only wrapper behavior. (seal review phase 4, 2026-05-15)
