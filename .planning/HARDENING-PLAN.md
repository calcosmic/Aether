...
| 2026-08-15 | **H5 / Phase 184 — one worker owns a run of file work** | **Done, with one deliberate limit.** A chain of dependent single-task waves sharing a caste is now one worker. Verified on the real shape: six chained copy steps → 1 worker (was 6); three independent tasks still fan out to 3; a chain that changes caste stays split. **Worktree mode is excluded on purpose** — there each worker gets its own copy of the repo and conflicts are keyed to declared paths, so merging changes what "this worker produced this file" means. The saving lands in the default in-repo mode, which is what CalVault used. **The existing suite caught a bug the new tests missed:** a merged worker that finished left every step after the first marked unfinished, so the phase could never advance. Task completion now reads every step a worker covered; regression tests added. Full suite green. Published as part of v1.0.54. |
| 2026-08-15 | **H4 / Phase 183 — the worker limit actually limits** | **Done, partial by design.** Root cause was not what it looked like: the budget is already per-command (`beginRuntimeSpawnRun` starts a fresh run for build, continue, plan and colonize). The carry-over came from ghosts — a worker whose completion failed to record stays live, and Phase 173's tamper rules deliberately count live entries outside the window, so one failed completion taxes every later command. `spawn-complete` no longer errors on a missing entry. Builds now check the whole plan against the remaining budget before the first worker starts and refuse in plain English rather than stalling half way. **Criterion 2 needs no change** — per-command reset already exists; it was ghosts, not scoping. **Criterion 3 (the failure log stops recording counts above the cap) is not separately verified** — it should follow from the preflight, but no test asserts the log itself. |
| 2026-08-15 | **H3 / Phase 182 — specialists earn their seat** | **Done.** Two confirmed defects, both measured against the live runtime with the real CalVault phase text. (1) The security specialist scored **0 relevance** and was **not required** — yet ran for 107,155 tokens, because the Queen asked and nothing refused it. The rule already existed one file away and only the deterministic engine consulted it. A proposed caste scoring zero is now refused and named in the output; required castes are exempt. (2) The test-coverage specialist was **mandatory** on a phase with no code, because the documentation-only check needs one of eight documentation words and "copy markdown files into a folder tree" has none. It now also asks whether the repository contains program code at all. Controls hold: real code work keeps its coverage specialist, a password-reset phase keeps its security reviewer, and the Watcher can never be refused. Full suite green. Published. |
