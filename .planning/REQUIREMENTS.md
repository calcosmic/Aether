...
- [ ] **WORK-02 — Proportionate ordinary work:** A one-task low-risk change uses one Builder plus free program checks; optional castes appear only for named evidence or risk, and explicit heavy review still exercises the independent-review pipeline.
...
- [ ] **WORK-05 — Complete result truth:** Success, no-change, partial work, blockers, flags, findings, deterministic checks, independent review, elapsed time, reported or unreported cost, and changed-file precision persist against the exact attempt and govern advance.
- [ ] **WORK-06 — Bounded recovery and learning:** Failures write canonical midden evidence, relevant blockers and signals; repair or Swarm waves checkpoint first, remain bounded, verify the repair, and restore the checkpoint if verification fails.
- [ ] **WORK-07 — Goal-level autopilot:** From one accepted goal, the controller can select survey, planning, work, verification, repair, replan, and ready-to-seal transitions automatically, stopping only for a declared owner, authority, physical, or unrecoverable boundary.
- [ ] **WORK-08 — Measured worker turnaround:** The runtime records per-job queue, preflight, model, tool-call, context, work, verification, and wait time; workers receive only task-relevant context plus compact handoffs, and representative one-plan latency is reduced through test architecture, model/routing, and brief improvements without weakening TDD or safety gates.
