...
**Success Criteria** (what must be TRUE) — to be settled at planning time; two candidate approaches, and the choice between them is a real decision, not a formality:
  1. *(Architectural — removes the gap)* The gate runs in its own job or workflow with no untrusted step ahead of it on the same runner, so there is no earlier step left to audit. Costs a second Go toolchain setup in CI; makes the guarantee structural rather than inspected.
  2. *(Interim — narrows the gap)* The shape audit is extended to scan every step's `run:` body in the gate's job for `$GITHUB_ENV` / `$GITHUB_PATH` writes and `go`-binary replacement, failing on any hit outside a reviewed whitelist. Cheaper, but it is blocklist-shaped over an adversarial surface — the failure mode this phase already hit four times running.
**Note for whoever plans this**: Phase 172 hit an unbounded tail because two of its criteria were written as absolutes over an adversarial surface. Write this phase's criteria as *bounded* claims from the outset, or accept approach 1, which is the only option that makes the absolute form honestly provable.
...
| 173. Delegation Guard | v1.26 | 10/10 | Complete   | 2026-08-13 |
