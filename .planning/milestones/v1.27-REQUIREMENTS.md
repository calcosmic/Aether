# Requirements Archive: v1.27 The Queen Decides, the Program Checks
...
- [x] **STAM-01**: Headless autopilot queues runtime-verification and visual checkpoints as pending decisions and continues; only genuine blockers halt the run
- [x] **STAM-02**: The named pause contract is restored — auditor score floor, critical audit finding, runtime verification needed, escalated flags — and `aether run --dry-run` lists every trigger by name
- [x] **STAM-03**: The replan pause reports learnings accumulated since the last plan
- [x] **STAM-04**: The run summary reports elapsed wall-clock time alongside the cost line
- [x] **STAM-05**: The pre/post-build blocker-count gate (`flag-check-blockers`) is live in run and status
- [x] **STAM-06**: A fixture colony of ≥ 6 phases with simulated workers completes unattended under one `aether run`; the test fails if any phase halts for a queueable reason

## Future Requirements (deferred, not abandoned — ratified order behind v1.27)

- Classic Colony Restoration: use `.aether/dreams/2026-09-01-comprehensive-aether-colony-review.md` as the authoritative owner brief for the functional and experiential February–April restoration; do not require Callum to re-disposition 69 audit rows before the milestone starts.
- Preserve the 72-row Classic capability ledger as traceability input during v1.28 roadmapping. It protects historical behavior from accidental deletion; it is not a standalone owner-question phase.
- Owner-watched proof after restoration: compare a small real task with plain Claude, run at least three phases overnight, and resume in a fresh session without re-briefing. Final scope and requirement IDs belong to v1.28 planning, not v1.27.
...
| STAM-01 | Phase 198.3 | Complete |
| STAM-02 | Phase 198.3 | Complete |
| STAM-03 | Phase 198.3 | Complete |
| STAM-04 | Phase 198.3 | Complete |
| STAM-05 | Phase 198.3 | Complete |
| STAM-06 | Phase 198.3 | Complete |
**Coverage:**

- v1.27 requirements: 48 total
- Mapped to phases: 48
- Unmapped: 0 ✓

---
*Requirements defined: 2026-08-22*
*Last updated: 2026-09-02 after the owner closed v1.27 scope at Phase 198.3 and moved Classic restoration/proof into or behind v1.28 (48/48 active v1.27 requirements complete)*
