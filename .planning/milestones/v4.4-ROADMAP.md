...
Plans:
- [x] 28-01-PLAN.md — Add safe-to-clear messages to build/continue/colonize and pheromone-first suggestions to colonize
- [x] 28-02-PLAN.md — Add auto-continue --all loop to continue.md with Task tool build delegation
...
All 24 v1 requirements mapped and satisfied:
...
**Key Decisions:**
- Three-guard pheromone decay pattern (clamp, cutoff, cap) applied consistently across 3 subcommands
- Queen-mediated spawn delegation (workers signal via SPAWN REQUEST, Queen fulfills) instead of direct subagent spawning
- Caste reuse pattern: reviewer reuses watcher-ant.md, debugger reuses builder-ant.md, archivist reuses architect-ant.md
- Two-tier learning with manual-only promotion and 50-entry global cap
- Adaptive complexity LIGHTWEIGHT/STANDARD/FULL set at colonization time
- Auto-continue delegates via Task tool (reads build.md) rather than inlining build logic

**Issues Resolved:**
- Pheromone decay math growing instead of decaying (field note 17)
- Activity log overwriting between phases (field note 19)
- Error tracking missing phase attribution (field note 18)
- Decision log unused during execution (field note 20)
- CP-1 resolved: Queen-mediated delegation works within platform constraints

**Issues Deferred:**
- NPM packaging/distribution (deferred until core stabilizes)
- External repo deployment model (deferred until core stabilizes)

**Technical Debt Incurred:**
- pheromones.md documents global learning source as 'auto:colonize' but implementation uses 'global:inject'
- validate-state does not check mode/mode_set_at/mode_indicators fields (optional with fallback)
- Command count grew to 14 files (organize.md added per FLOW-02 requirement)
