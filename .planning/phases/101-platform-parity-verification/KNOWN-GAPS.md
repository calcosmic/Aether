...
**Finding:** Of the 60 YAML commands, 27 have corresponding Codex TOML agent files in `.codex/agents/`. The remaining 33 commands have no TOML agent.

**Decision:** Accepted.

**Rationale:** TOML agents represent worker castes (builder, watcher, scout, etc.), not command wrappers. Adding 33 wrapper TOML files would duplicate the command-guide surface without adding value. All 60 commands are covered by the `commandGuideCatalog()` function which provides Codex orchestration guidance.
