...
| `.aether/docs/PARITY_CLASSIC_VS_GO.md` | Current parity guide calls `aether recover` the delivered modern E2E route. | Plan 199-34 rewrites the current proof to use read-only maintenance inspection plus canonical resume and executes the same contract. |
| `cmd/hook_cmds.go` | Active session-stop grace logic compares persisted `LastCommand` directly with `resume-colony`. | Plan 199-34 normalizes legacy stored input at the single `cmd/normalize_args.go` boundary and requires canonical `resume` downstream. |
| `cmd/recovery_engine.go` | Active routing contains its own `resume-colony` alias switch and can preserve a second internal identity. | Plan 199-34 removes the local alias map and tests canonical recovery candidates/result fields over normalized input. |
| `cmd/recover_visuals.go` | Active human/next-action output emits `aether recover --apply` and force variants. | Plan 199-34 replaces emitted repair directions with read-only `aether maintenance recovery-inspect` or exact `aether resume` according to effect. |
| `cmd/recover_repair.go` | The current mutation entry is explicitly organized around public `recover --apply`. | Plan 199-34 retires the independent entry and preserves useful repairs only through the previewed, receipted maintenance transaction from Plan 199-18. |
| `cmd/worktree.go` | Active preservation routing leaves work discoverable through `aether recover`. | Plan 199-34 points inspection to `aether maintenance recovery-inspect`, retains branch/path evidence, and returns runnable lifecycle restoration to resume. |
| `README.md`, `AGENTS.md`, `.aether/rules/aether-colony.md`, `.claude/rules/aether-colony.md`, `.aether/templates/opencode-md-template.md`, `cmd/.opencode/OPENCODE.md`, `.aether/templates/agents-md-template.md`, `cmd/AGENTS.md` | These eight current guide/template/generated surfaces contain or can regenerate seal→entomb/init sequencing that contradicts retained reviewable seal state. | Plan 199-33 makes status/review primary, entomb explicitly optional, and init conditional on successful archive-and-clear; `TestCurrentVocabularyDocs199` verifies both canonical/generated pairs through production dry runs. |

The tracked-working-tree audit uses `git grep`/`git ls-files`, not an untracked-directory scan, for literal `pause-colony`, `resume-colony`, `aether recover`, `/ant-recover`, `recover --apply`, and post-seal status/entomb/init sequencing. Each discovered occurrence must land in exactly one of the following buckets; there is no “other occurrence” escape hatch. [VERIFIED: owner-authorized tracked-repository audit on 2026-09-03]

| Inventory bucket | Required treatment |
|---|---|
| `active_current_documentation_template_generated` | Exact path plus canonical source/generated relation and owning migration plan; retired command-shaped teaching forbidden and production dry-run parity required. |
| `active_runtime_state_routing` | Exact path plus owning implementation plan and executed route proof; canonical downstream values are `pause`/`resume`, and no retired public recovery suggestion may remain. |
| `hidden_parser_compatibility` | Only exact old input tokens in `cmd/normalize_args.go`, invisible to help/output/persisted downstream state, with machine-checked expiry 1.29. |
| `historical_evidence_versioned_fixture_test_input` | Exact file and narrow purpose for every retained match (quoted history, forbidden-token assertion, or legacy-input fixture); directory, prefix, and glob allowlists forbidden. |

Plan 199-28 owns the independent occurrence-level inventory and compares its exact path/token/count-or-locator rows with a fresh tracked-file scan. It fails when a match is absent from the inventory, multiply classified, counted differently, placed outside its declared bucket, or exempted by a broad path. It separately fails for forbidden spelling in a current surface, generated output that differs from a production dry run, a noncanonical persisted/runtime route, or an executed suggestion that reaches a retired recovery door. `TestCurrentVocabulary199` also independently checks the eight D-17 surfaces for status-primary ordering, optional entomb, post-clear-only init, forced-marker retention, and generated-current parity. [VERIFIED: D-13/D-15/D-17 + second owner-authorized checker revision]

The earlier retry found that exact `go test -run` selectors can succeed when no test matches. Plan 199-02 therefore defines the meaningful aggregate `TestCommandSourceHygiene` before every consuming plan. Plan 199-29 independently proves source definitions exist for corpus, parity, coverage, current vocabulary, document/D-17 vocabulary, runtime recovery routes, persisted-state recovery compatibility, gate schema, aggregate source hygiene, retired-alias pruning, cleanup ownership, and `pkg/colony` lifecycle families before it starts the focused lane. [VERIFIED: Go `-run` zero-match behavior + targeted checker recommendation]
