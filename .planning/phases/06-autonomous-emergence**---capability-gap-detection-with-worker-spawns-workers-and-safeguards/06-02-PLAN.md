...
for worker in builder-ant colonizer-ant route-setter-ant scout-ant watcher-ant architect-ant; do
  ...
  if ! grep -q "Context Inheritance Implementation" ".aether/workers/${worker}.md"; then
    echo "FAIL: ${worker} missing context inheritance wiring"
    exit 1
  fi
...
- Context inheritance implementation shows jq commands to load pheromones
- Context inheritance implementation shows jq commands to load working memory
- Context inheritance implementation shows jq commands to load constraints
</done>
</task>
...
3. **Worker Ant prompts updated**
   ...
   - record_spawn() called before Task tool
   - record_outcome() called after specialist completes
   - Context inheritance implementation shows explicit jq wiring
...
- Context inheritance uses explicit jq commands to load pheromones and memory
</success_criteria>
...
- Context inheritance wiring with jq examples
