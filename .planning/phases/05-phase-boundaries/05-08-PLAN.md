...
must_haves:
  truths:
    ...
    - "Pheromone emission uses direct jq updates (no wrapper functions needed)"
  artifacts:
  ...
  key_links:
    ...
    - from: "adapt_next_phase_from_memory()"
      ...
      via: "Emits FOCUS/REDIRECT pheromones via direct jq updates"
      pattern: "jq.*active_pheromones.*FOCUS|REDIRECT"
    - from: "adapt_next_phase_from_memory()"
...
Purpose: Before planning next phase, colony reads previous phase's compressed memory and high-value patterns, then adapts next phase approach by emitting appropriate FOCUS and REDIRECT pheromones via direct jq updates.

Output: adapt_next_phase_from_memory() function that reads memory, extracts patterns, emits pheromones (via jq), stores adaptation in colony state
</objective>
...
5. **Adapt next phase by emitting pheromones via direct jq updates:**
...
- Use direct jq updates to emit pheromones (NOT wrapper functions)
- Phase 3 created .md command files, not bash functions
...
**Why direct jq updates:** Phase 3 created pheromone emission as .md command files (focus.md, redirect.md), not bash functions. The adapt_next_phase_from_memory() function cannot call these as functions. Instead, it directly updates pheromones.json using jq, which is the same approach used by the Phase 3 commands internally.
  </action>
  <verify>
```bash
# Source dependencies
...
```
  ...
  <done>
adapt_next_phase_from_memory() reads previous phase patterns, emits FOCUS/REDIRECT pheromones via direct jq updates, stores adaptation in colony state
  </done>
</task>
...
4. FOCUS pheromones emitted for focus_preferences via direct jq updates
5. REDIRECT pheromones emitted for constraints via direct jq updates
...
11. Pheromone emission uses direct jq updates (no wrapper function calls)
</verification>
...
3. FOCUS/REDIRECT pheromones emitted via direct jq updates based on patterns
...
9. No dependency on non-existent wrapper functions
</success_criteria>
...
- Pattern extraction and pheromone emission tested (via jq)
- Adaptation storage in colony state documented
- Direct jq approach used (no wrapper functions)
- Files modified
</output>
