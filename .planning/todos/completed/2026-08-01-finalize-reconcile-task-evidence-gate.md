...
title: "[CLOSED 2026-08-22] continue-finalize does not count --reconcile-task as recorded reconciliation for the implementation_evidence gate"
area: cmd/continue
...
## Closed (2026-08-22, Phase 193 Plan 04)

`continue-finalize` now registers `--reconcile-task` (unioned with any IDs
already recorded when the build was planned), and a reconciled task with a
passing deterministic floor advances even with no builder claims file, on
both the direct `aether continue` path and the `continue-finalize` wrapper
path. Closed by `cmd/floor_reviewer_free_gate_test.go`'s
`TestFinalizeCountsReconcileTaskAsEvidence` (proves both paths reach the same
verdict from identical inputs) and `TestReconcileIsNotABypass` (proves a
reconciled task whose checks genuinely fail still blocks, on both paths).
