...
Read `.aether/data/events.json`.

**Only if Step 4 actually ran** (learnings were extracted, not skipped due to auto-extraction detection), append a `learnings_extracted` event:
...
**Always** append a `phase_advanced` event:
