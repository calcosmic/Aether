...
If go test or bash test suites fail to run:
...
```bash
# Count test files
test_count_go=$(find . -name "*_test.go" -not -path "./vendor/*" 2>/dev/null | wc -l | tr -d ' ')
echo "go=$test_count_go"
echo "total=$test_count_go"
```
