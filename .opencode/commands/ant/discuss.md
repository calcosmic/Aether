...
OpenCode may use a native question card for those structured choices, but it
may not add, drop, rewrite, classify, or answer a card. Nothing is recorded
without the owner's explicit pick. Submit the pick only through the card's
exact `aether discuss --resolve <id> --answer "<answer>"` syntax.
