...
4. Do NOT spawn workers — the briefing already holds the colony's memory; a
   spawn adds latency and no information. Do NOT invent beyond the briefing:
   when the colony has no record of something, say exactly that ("the colony
   doesn't have a record of that — try `/ant-history` or ask me to dig").
5. If the question is about the CODE rather than the colony (how a function
   works, where something is implemented), say so and point to
   `/ant-quick "<question>"`, which dispatches a read-only scout at the repo.
