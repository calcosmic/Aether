...
- **Global scope**: `/Users/callumcowie/repos/Aether/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/callumcowie/repos/Aether/.claude/notes/`)
...
2. Glob `/Users/callumcowie/repos/Aether/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/callumcowie/repos/Aether/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/callumcowie/repos/Aether/.claude/notes/` — works in any directory
