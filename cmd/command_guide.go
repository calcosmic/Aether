...
func codexNativeBuildCommandGuide(def commandGuideDefinition) commandGuideDefinition {
	...
	for _, step := range def.PreSteps {
		if strings.Contains(step, "build-wave playbook") || strings.Contains(step, "Spawn parallel waves") || strings.Contains(step, "Pass worker briefs verbatim") || strings.Contains(step, "After all terminal results") || step == finalizerCompletionContractStep("build") {
			continue
		}
	...
	def.PreSteps = append(steps,
		...
		"Write strict JSON requests into an absolute regular file in a new aether-worker-request-* directory under the system temporary directory. Every request requires schema_version:1, phase and the exact manifest execution_binding. For each accepted dispatch run `aether codex-native-worker reserve --request <file>` with worker_name, task_id, actual host_session_id, canonical workspace and host_permission:workspace_write. Use the dispatch's nonempty task_id; for auxiliary jobs with no raw task_id use the runtime normalizedDispatchTaskID convention: trimmed stage-caste-name joined by hyphens, lowercased, spaces replaced with hyphens, outer hyphens trimmed. Preserve the accepted manifest bytes. Unsupported workspace, permissions or require_governed_nesting:true refuse before spawning; never silently switch lanes.",
		"Only a fresh launch_allowed:true response permits one spawn_agent call. Use the response dispatch agent_name/caste and worker name, with fork_turns:none for child-fetch/v1, and pass worker.native.prompt bytes verbatim. The child must wait without editing or running checks until released. Save the actual returned child ID. Replayed or unresolved reservations never authorize another spawn.",
		"Run `aether codex-native-worker bind --request <file>` with the same execution_binding, worker_name, task_id, host_session_id, worker.provider_run_id as launch_id, actual child_id, dispatch_sha256 and prompt_sha256 from worker.native. Only after bind succeeds send worker.native.release verbatim to that child using the native messaging tool. For child-fetch/v1, complete the child context read and separate ACK before useful work; retain the original protocol for saved legacy attempts.",
		"After required context_fetched observations are durable for child-fetch/v1, immediately run `aether codex-native-worker record --request <file>` with all bound identity fields, result (the child's actual nonempty JSON result), source_event_id and source_event_sha256 of the retained raw terminal event. Retain raw parent/child tool events. Unknown outcomes remain unresolved. Never substitute parent edits or manufactured results for the child.",
		"Run `aether codex-native-worker stage --request <file>` with schema_version, phase and execution_binding only after required terminal results are saved; use result.completion_path. The exact saved-binding inspect --request route is read-only. No helper is mandatory beyond the runtime-selected team.")
	def.PreSteps = append(def.PreSteps,
		"Fresh-parent recovery begins with public `aether resume`, then `aether codex-native-worker inspect --phase <phase>`. Retain finished results unchanged. Execute only saved never-started assignments that the runtime admits; never replace unresolved launches. Reconnect only to the same actual child with host evidence. `aether codex-native-worker observe --request <file>` records actual running, unavailable or launch_unresolved evidence. An actual interrupt request records observation_status:cancel_requested. A cancellation request is not a cancelled terminal: only an actual host cancellation acknowledgement permits observation_status:cancelled. Native worktree allocation and governed recruitment are unsupported. Never copy results into legacy per-worker subprocess result files: the native journal and stage are authoritative.",
		"Raw child token_usage_record events may expose provider usage, but native worker usage is currently uncollected by Aether. Empty saved Usage is unreported, not zero cost. Do not submit worker prose or parent totals as child measurements. Encrypted host messages establish call/child linkage, not independently verified exact plaintext delivery.",
	...
	def.PreSteps = append(def.PreSteps,
		"New accepted native manifests pin context_protocol:child-fetch/v1. After binding, give that same child an absolute metadata-only request file with its exact bound fields and context_purpose:initial. Keep this purpose-specific copy separate from the original parent question/observe/record request; use it only for context/context-ack. The child executes the absolute pinned aether executable with codex-native-worker context --request <file>, returning the full Go JSON unchanged with sufficient output capacity. Its initial payload is the exact saved full prompt, not a reconstructed brief. Missing or truncated output is not delivery.",
		"The child then makes a separate host tool call to the same absolute executable: codex-native-worker context-ack --request <file> --delivery-id <ID from completed read> --payload-sha256 <hash from completed read>, plus one --decision-id <ID> for each fetched decision ID in order. Never prepopulate an ACK or combine read and ACK in one tool call. After successful ACK the child may work; no extra parent handshake is required. Fetch and ACK are read-only and grant no credit.",
		"Before accepting a successful terminal, parent observes the actual read and ACK from the uniquely bound child raw rollout under the selected CODEX_HOME/sessions or exact host-supplied raw path. Verify child/parent/task/role/workspace metadata, unique literal exec call, successful matching CommandExecution and full unchanged returned tool output. Each read/ack source carries child_id, turn_id, call_id, started_at, completed_at and call/command/result:{id,sha256} using actual raw payload/item IDs and bare 64-character hexadecimal SHA-256 hashes of original lines excluding newline. Read result precedes separate ACK call, and ACK precedes relevant work/checks. Missing source remains unproved.",
		"Run observe with observation_status:context_fetched, the exact context_delivery, context_fetch:{schema_version:1,read:<source>,ack:<source>}, and the ACK result timestamp/ID/hash as observed_at/source_event_id/source_event_sha256. Keep observations in source chronology and persist before successful terminal acceptance; parent observation may occur after the child work. Never report a fetched payload as a legacy completed send. New protocol terminal success requires the initial receipt and applicable answer receipts; only the existing finalizer grants credit.",
		"For an answered current child-fetch/v1 child, create a fresh metadata-only bound request with context_purpose:answers and send only its pointer to that same child. It reads and separately ACKs the exact scoped answer and fetched decision-ID set before answer-dependent work; parent records actual context_fetched evidence. Do not expose answers in bootstrap or an unbound message. Saved attempts with no protocol keep the historical host-send rules; never switch an in-flight attempt or silently fall back when the required runtime operation is unavailable.",
	)
	...
	post := make([]string, 0, len(def.PostSteps))
	for _, step := range def.PostSteps {
		if !strings.HasPrefix(step, "In worktree mode one job") {
...
func commandGuideCatalog() map[string]commandGuideDefinition {
	...
	catalog["plan"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow plan --manifest-file <manifest file>`.",
			"Render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow plan --manifest-file <manifest file> --execution-wave <execution_wave>` for the one current stage.",
	...
	catalog["colonize"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Save the full JSON envelope to a temporary manifest file, then render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow colonize --manifest-file <manifest file>`.",
			...
			"Before each manifest wave, render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow colonize --manifest-file <manifest file> --execution-wave <execution_wave>`.",
	...
	catalog["swarm"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Save the full JSON envelope to a temporary manifest file, then render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow swarm --manifest-file <manifest file>`.",
			...
			"Before each manifest wave, render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow swarm --manifest-file <manifest file> --execution-wave <execution_wave>`.",
	...
	catalog["build"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Before rendering spawn ceremonies or spawning workers, inspect `result.orchestrator_boundary_guidance` and the matching manifest `orchestrator_boundary_guidance`: if active or `next` is `aether discuss`, stop, show the summary, route to `aether discuss`, tell the user to rerun `after_discuss_next`, and request a fresh plan-only manifest after the answer is resolved.",
			"Render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow build --manifest-file <manifest file>` for the old-style caste-colored spawn ceremony.",
			...
			"Before each manifest wave, render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow build --manifest-file <manifest file> --execution-wave <execution_wave>`.",
	...
	catalog["continue"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"For heavy external review, render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow continue --manifest-file <manifest file>`.",
			"Before each heavy review wave, render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow continue --manifest-file <manifest file> --execution-wave <execution_wave>`.",
	...
	catalog["seal"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Before rendering spawn ceremonies or spawning workers, inspect `result.orchestrator_boundary_guidance` and the matching manifest `orchestrator_boundary_guidance`: if active or `next` is `aether discuss`, stop, show the summary, route to `aether discuss`, tell the user to rerun `after_discuss_next`, and request a fresh plan-only manifest after the answer is resolved.",
			"Render `AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow seal --manifest-file <manifest file>`.",
			...
			"Before each final-review wave, render `AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow seal --manifest-file <manifest file> --execution-wave <execution_wave>`.",
