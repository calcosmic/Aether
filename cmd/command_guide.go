...
func commandGuideCatalog() map[string]commandGuideDefinition {
	...
	catalog["plan"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Save the full JSON envelope to a temporary manifest file, then render `AETHER_FORCE_COLOR=1 AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow plan --manifest-file <manifest file>`.",
	...
	catalog["build"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Save the full JSON envelope to a temporary manifest file, then render `AETHER_FORCE_COLOR=1 AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow build --manifest-file <manifest file>` for the old-style caste-colored spawn ceremony.",
	...
	catalog["continue"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"For heavy external review, save the full JSON envelope to a temporary manifest file and render `AETHER_FORCE_COLOR=1 AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow continue --manifest-file <manifest file>`.",
	...
	catalog["seal"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Save the full JSON envelope to a temporary manifest file, then render `AETHER_FORCE_COLOR=1 AETHER_OUTPUT_MODE=visual aether ceremony spawn-plan --workflow seal --manifest-file <manifest file>`.",
