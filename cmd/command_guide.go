...
func commandGuideCatalog() map[string]commandGuideDefinition {
	...
	catalog["build"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Run `aether host build <phase>` to fetch the dispatch manifest via the TS host. Parse `result.dispatch_manifest`; do not parse visual output.",
			"Save the full JSON envelope to a temporary manifest file for later ceremony rendering.",
	...
	catalog["continue"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"For heavy external review, run `aether host continue --verification-depth heavy $ARGUMENTS` to fetch the manifest via the TS host. Parse `result.continue_manifest`; do not parse visual output.",
