...
func commandGuideCatalog() map[string]commandGuideDefinition {
	...
	catalog["oracle"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Ask the user to choose research depth unless they already gave one: quick, balanced, deep, or exhaustive.",
		...
		RunCommand: "AETHER_OUTPUT_MODE=visual aether oracle --depth <depth> --confidence-target <percent> --template <template> \"<synthesized prompt>\"",
		PostSteps: []string{
