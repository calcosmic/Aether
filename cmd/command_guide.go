...
func commandGuideCatalog() map[string]commandGuideDefinition {
	...
	catalog["plan"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Run `aether host plan --depth <choice> --planning-depth <choice>` to fetch the manifest via the TS host. If the TS host is unavailable, fall back to `AETHER_OUTPUT_MODE=json aether plan --plan-only --depth <choice> --planning-depth <choice>`. Parse `result.plan_manifest` or `result.planning_manifest`.",
	...
	catalog["build"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"Run `aether host build <phase>` to fetch the dispatch manifest via the TS host. If the TS host is unavailable, fall back to `AETHER_OUTPUT_MODE=json aether build <phase> --plan-only`. Parse `result.dispatch_manifest`; do not parse visual output.",
			"Save the full JSON envelope to a temporary manifest file for later ceremony rendering.",
			"Before rendering spawn ceremonies or spawning workers, inspect `result.orchestrator_boundary_guidance` and the matching manifest `orchestrator_boundary_guidance`: if active or `next` is `aether discuss`, stop, show the summary, route to `aether discuss`, tell the user to rerun `after_discuss_next`, and request a fresh plan-only manifest after the answer is resolved.",
	...
	catalog["continue"] = commandGuideDefinition{
		...
		PreSteps: []string{
			...
			"For heavy external review, run `aether host continue --verification-depth heavy $ARGUMENTS` to fetch the manifest via the TS host. If the TS host is unavailable, fall back to `AETHER_OUTPUT_MODE=json aether continue --plan-only --verification-depth heavy $ARGUMENTS`. Parse `result.continue_manifest`; do not parse visual output.",
