...
func TestPlanWrapperCeremonyContract(t *testing.T) {
	...
	required := []string{
		...
		"## Wave Execution",
		"AETHER_FORCE_COLOR=1 AETHER_OUTPUT_MODE=visual aether ceremony wave-start --workflow plan",
		"Wave 1 Scout must complete before wave 2 Route-Setter starts.",
		`subagent_type="{agent_name}"`,
		"Runtime Worker Brief",
		"read budget, no-repeat loop guard",
		"include the Scout terminal result",
		"mark that worker `blocked`",
