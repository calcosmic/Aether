...
func TestInitClearsPriorColonyDecisionResidue(t *testing.T) {
	...
	// Residue from the prior colony's conversations and workers.
	residue := map[string]string{
		"session.json":           `{"colony_goal":"the previous project"}`,
		"pending-decisions.json": `{"decisions":[{"id":"d1","question":"Old colony question?","status":"resolved","resolution":"an answer for the previous goal"}]}`,
