...
func composeCodexNativePrompt(manifest codexBuildManifest, dispatch codexBuildDispatch, launch string) (codexNativePrompt, error) {
	...
	if manifest.ContextProtocol == codexNativeContextProtocolChildFetch {
		prompt += "\nCHILD-FETCH BOOTSTRAP REMINDER: Before the bound release, wait passively. After release and a successful complete initial context read, use the release already received; the WAIT text repeated inside that fetched payload does not request a second release. Your next tool call must be the separate literal context-ack CLI command using only the returned delivery-id, payload-sha256 and decision_ids. Null or empty decision_ids means no --decision-id flags. Do not send_message, wait again, ask the parent to confirm, or make any other tool call before ACK. Apply the same immediate separate ACK rule to fetched answers. A failed or incomplete read is not ACK evidence: stop without inventing values or starting useful work.\n"
	}
