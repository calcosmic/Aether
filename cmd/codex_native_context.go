...
func composeCodexNativePrompt(manifest codexBuildManifest, dispatch codexBuildDispatch, launch string) (codexNativePrompt, error) {
	...
	if manifest.ContextProtocol == codexNativeContextProtocolChildFetch {
		prompt += "While waiting, remain idle, wait passively with collaboration.wait_agent({\"timeout_ms\":3600000}), or send a brief commentary message; do not use assignment tools, submit a final answer or ask a material question. After release, before assignment work or questions, use the parent-provided bound initial-context request with codex-native-worker context; read its entire payload. In a separate tool invocation call context-ack with that returned delivery-id, payload-sha256 and each decision-id. Fetch answers and separately ACK their exact bytes before answer-dependent work. A read alone is not an acknowledgement. Never fill ACK fields from parent-provided claims or write parent observations yourself.\n"
	}
