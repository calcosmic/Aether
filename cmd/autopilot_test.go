...
func TestAutopilotCheckReplan(t *testing.T) {
	saveGlobals(t)
	resetRootCmd(t)
	...
	// Initialize and complete 3 phases
	rootCmd.SetArgs([]string{"autopilot-init", "--phases", "5"})
	_ = rootCmd.Execute()
	...
	// Check replan with interval 3
	resetRootCmd(t)
	stdout = &buf
	buf.Reset()
	rootCmd.SetArgs([]string{"autopilot-check-replan", "--interval", "3"})
	if err := rootCmd.Execute(); err != nil {
	...
	if result["replan"] != true {
		t.Fatalf("expected replan=true after 3 completed phases with interval 3, got: %v", result["replan"])
	}
	if result["completed"] != float64(3) {
		t.Fatalf("expected completed=3, got: %v", result["completed"])
	}
}
