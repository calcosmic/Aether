...
func TestInitResearchCharterExpanded(t *testing.T) {
	saveGlobals(t)
	resetRootCmd(t)
	var buf bytes.Buffer
	stdout = &buf
	...
	// Verify the 3 new fields are present and non-empty
	if ts, ok := charter["tech_stack"].(string); !ok || ts == "" {
		t.Errorf("charter.tech_stack = %v, want non-empty string", charter["tech_stack"])
	}
	if kr, ok := charter["key_risks"].(string); !ok || kr == "" {
		t.Errorf("charter.key_risks = %v, want non-empty string", charter["key_risks"])
	}
	if cs, ok := charter["constraints"].(string); !ok || cs == "" {
		t.Errorf("charter.constraints = %v, want non-empty string", charter["constraints"])
	}
}

func TestInitResearchCharterKeyRisksNoCI(t *testing.T) {
	saveGlobals(t)
	resetRootCmd(t)
	var buf bytes.Buffer
	stdout = &buf
	...
	rootCmd.SetArgs([]string{"init-research", "--goal", "no CI test", "--target", target})
	...
	keyRisks := charter["key_risks"].(string)
	if !strings.Contains(keyRisks, "No CI/CD pipeline") {
		t.Errorf("charter.key_risks = %q, want to contain 'No CI/CD pipeline'", keyRisks)
	}
}

func TestInitResearchCharterConstraintsWithLinter(t *testing.T) {
	saveGlobals(t)
	resetRootCmd(t)
	var buf bytes.Buffer
	stdout = &buf
	...
	rootCmd.SetArgs([]string{"init-research", "--goal", "linter constraints test", "--target", target})
	...
	constraints := charter["constraints"].(string)
	if !strings.Contains(constraints, "ESLint") {
		t.Errorf("charter.constraints = %q, want to contain 'ESLint'", constraints)
	}
}
